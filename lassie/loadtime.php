 <?php
	...
	$sql_days = "select * from days where compcode = 'lassie' and day_code = '".$shotdays."'"or die("Error:". mysqli_error($conn));
	
		$result_days = mysqli_query($conn,$sql_days);
		while (($row_days = mysqli_fetch_array($result_days))){
		  $data_days[] = $row_days; // add the row in to the results (data) array
		}
		// print_r( $data_days);
		if ($data_days[0]['status'] == 'inactive') {?>
...
<?php }else{
   ...
   $sql = "
			...
			FROM
				   ...
				   appointments
				   ON 
					   set_time.days = appointments.`shotday` and
					   set_time.time = TIME_FORMAT(appointments.stime,'%H:%i')
			   WHERE
				   set_time.days = '" . $days[$day] . "' AND 
				status ='active' GROUP BY set_time.time,set_time.days order by time ASC
	  ...
	  if($scheduled_day == date('Y-m-d')){
	   ...
	   // echo $time.">".$datatime;
		  ...
		  <?php }elseif ($result['start']=="") {?>
				  <?php if ($result['booking_status'] == "booking") {?>
					  
				<?php }else{
				// echo $data_days[0]['status'];
				?>
				  <div class="d-flex flex-stack gap-5 mb-3">
