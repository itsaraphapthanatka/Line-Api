<?php
   ...
   // if( $_GET["name"]) {
	...
	//   }
	  ...
	  sendLineNotify_customer($message,$fullname,$name,$userid,$date,$time,$mobile);
 ...
 function sendLineMessage($userid,$name,$date,$time,$location,$mobile,$fullname){
	  ...
	  curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://appointportal.appreview.co.th/pushmessage.php',
 ...
 function sendLineNotify_customer($message,$fullname,$name,$userid,$date,$time,$mobile){
	
	 $token = "E8SzTRd8MvOW428UIcZG6cWSFAMge6RROkZUoN5phnV"; // ใส่ Token ที่สร้างไว้ BIG
	 // $token = "FruM0XKAjGpdBDGyc4c0bDT9VOoRYIdImoW6KCvflQc"; // ใส่ Token ที่สร้างไว้ TOP
	 // $token = "aMwvyw0qEzjc8D2Zak2t5Y3wEvgjFWVc5RPt1mdSkl2"; // ใส่ Token ที่สร้างไว้ AOF
	 $date = date("Y-m-d H:i:s");
	 $ch = curl_init();
